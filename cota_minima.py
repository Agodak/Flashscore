...
for j in range(10):
    ...
    plt.plot(money)
    plt.title('Bundesliga '+str(j+10)+'/'+str(j+11) +'. Minimal odd bet on every match')
    ...
    plt.grid(True)
    plt.savefig('plots_bundesliga/minimal_odd_every_match/Plot_bundesliga_'+str(j+10)+'-'+str(j+11)+'_minimal_odd.png')
    file.close()
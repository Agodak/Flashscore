...
def bully_scrap(sezon,salvare):
    ...
    if(len(meciuri)!=308 and len(meciuri)!=306):
    ...
    i=0
    if sezon!='https://www.flashscore.ro/fotbal/germania/bundesliga-2011-2012/rezultate/':
        meciuri=meciuri[2:]
    for meci in meciuri:
        ...
        try:
            ...
            tab_nou.implicitly_wait(10)
            meciuri_de_salvat[i]['fixture']=div_date.text.split()[4]
            print(meciuri_de_salvat[i]['fixture'])
            tab_nou.implicitly_wait(10)
            meciuri_de_salvat[i]['date_time']=div_date.text.split()[5], div_date.text.split()[6]
            ...
            print('am ratat meciul: '+str(len(meciuri_de_salvat))+ ' din sezonul:')
            print(sezon)
            continue
...
lista_link = ['https://www.flashscore.com/football/germany/bundesliga-20{}-20{}/results/'.format(i, i + 1) for i in range(10, 20)]
lista_salvari=['meciuri_bundesliga_{}-{}.txt'.format(i,i+1) for i in range(10,20)]

for j in range(10):
    bully_scrap(lista_link[j],lista_salvari[j])